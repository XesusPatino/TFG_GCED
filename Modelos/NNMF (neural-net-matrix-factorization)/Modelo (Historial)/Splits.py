...
# Paths
ml1m_dir = "C:/Users/xpati/Documents/TFG/ml-1m"
output_dir = os.path.join(ml1m_dir, "split")
ratings_file = os.path.join(ml1m_dir, "ratings.dat")

# Create output directory if it doesn't exist
os.makedirs(output_dir, exist_ok=True)

# Load ratings data
print("Loading ML-1M ratings data...")
ratings = pd.read_csv(
    ratings_file,
    sep="::",
    engine="python",
...
print(f"Original data - users: {ratings['user_id'].nunique()}, movies: {ratings['item_id'].nunique()}")
print(f"Min user_id: {ratings['user_id'].min()}, Max user_id: {ratings['user_id'].max()}")
print(f"Min movie_id: {ratings['item_id'].min()}, Max movie_id: {ratings['item_id'].max()}")

# Important: Enforce consistent user IDs and movie IDs
# Instead of just subtracting 1, remap all IDs to ensure they're contiguous from 0 to n-1
...
print(f"Remapped data - users: {num_users}, movies: {num_items}")
...
# Save the mapping for reference - might be useful for interpreting results later
