    ...
    def end_test(self, test_rmse, test_mae):
        ...
        if self.best_rmse_epoch is not None:
            ...
            if self.best_rmse_metrics:
                ...
                if 'valid_mae' in self.best_rmse_metrics and self.best_rmse_metrics['valid_mae'] is not None:
...
def train(model, train_data, valid_data, test_data, batch_size, max_epochs, use_early_stop, early_stop_max_epoch, 
    ...
    if use_history and isinstance(model, NNMFWithHistory):
        ...
        # Calculate also MAE for the history-adjusted evaluation
        ...
        test_mae = absolute_error_sum / max(1, mae_count)
        print(f"\nTest RMSE with history adjustment: {test_rmse:.4f}")
        print(f"Test MAE with history adjustment: {test_mae:.4f}")
        
        # Also calculate standard RMSE and MAE for comparison
        standard_rmse = model.eval_rmse(test_data)
        standard_mae = model.eval_mae(test_data)
        print(f"Standard Test RMSE (without history): {standard_rmse:.4f}")
        print(f"Standard Test MAE (without history): {standard_mae:.4f}")
        print(f"RMSE improvement from history adjustment: {standard_rmse - test_rmse:.4f}")
        print(f"MAE improvement from history adjustment: {standard_mae - test_mae:.4f}")
    else:
        # Standard evaluation
        test_rmse = model.eval_rmse(test_data)
        test_mae = model.eval_mae(test_data)
    
    system_tracker.end_test(test_rmse, test_mae)    # Save training history
    timestamp = time.strftime("%Y%m%d-%H%M%S")
