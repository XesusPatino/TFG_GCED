...
# Paths
ml1m_dir = "C:/Users/xpati/Documents/TFG/Bases_Extra/GDSC"
output_dir = os.path.join(ml1m_dir, "split")
ratings_file = os.path.join(ml1m_dir, "gdsc1_processed.csv")  # Update with your actual filename
...
# Read only the first three columns with headers
ratings = pd.read_csv(
    ratings_file,
    sep=",",
    header=0,  # File has header
    usecols=['user_id', 'item_id', 'rating'],  # Only use first three columns
    encoding="utf-8"
)
...
# Save splits - ensure no headers in output files
