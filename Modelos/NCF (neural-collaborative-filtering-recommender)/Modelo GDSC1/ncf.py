...
ratings_df = pd.read_csv('C:/Users/xpati/Documents/TFG/gdsc1_processed.csv')
...
print("Primeras filas del dataset GDSC1:")
...
# Mostrar información de las características adicionales
if 'avg_cosmic_response' in ratings_df.columns:
    print(f"Rango avg_cosmic_response: {ratings_df['avg_cosmic_response'].min():.4f} - {ratings_df['avg_cosmic_response'].max():.4f}")
if 'avg_drug_response' in ratings_df.columns:
    print(f"Rango avg_drug_response: {ratings_df['avg_drug_response'].min():.4f} - {ratings_df['avg_drug_response'].max():.4f}")
if 'eccentricity' in ratings_df.columns:
    print(f"Rango eccentricity: {ratings_df['eccentricity'].min():.4f} - {ratings_df['eccentricity'].max():.4f}")
...
# Obtener número de usuarios y items únicos
n_users = len(user_encoder.classes_)
n_movies = len(movie_encoder.classes_)

print(f'Número de usuarios únicos: {n_users}')
print(f'Número de items únicos: {n_movies}')
print(f'Total de interacciones: {len(ratings_df)}')
...
class NeuralCollaborativeFiltering(nn.Module):
    def __init__(self, n_users, n_items, embedding_dim=50, hidden_dim=128):
...
train_dataset = GDSC1Dataset(train_df)
val_dataset = GDSC1Dataset(val_df)
