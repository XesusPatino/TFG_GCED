...
def main():
    ...
    with tf.compat.v1.Session(graph=model.graph) as sess:
        ...
        for epoch in range(args.train_epoch):
            ...
            # Calcular métricas de ranking solo cada 5 épocas para ahorrar tiempo
            if epoch % 5 == 0 or epoch == args.train_epoch - 1:
                ...
                # Usar los últimos valores disponibles
                combined_recall = combined_recalls[-1] if combined_recalls else 0.0
                combined_ndcg = combined_ndcgs[-1] if combined_ndcgs else 0.0
                print(f"Recall@10 combinado: {combined_recall:.4f} (de época anterior)")
                print(f"NDCG@10 combinado: {combined_ndcg:.4f} (de época anterior)")
            ...
            # Solo calcular métricas de ranking combinadas cada 5 épocas o en épocas clave
            if epoch % 5 == 0 or epoch == args.train_epoch - 1:
                ...
                # Si no las calculamos, usar el último valor conocido
                combined_recall = combined_recalls[-1] if combined_recalls else 0.0
                combined_ndcg = combined_ndcgs[-1] if combined_ndcgs else 0.0
                print(f"Recall@10 combinado: {combined_recall:.4f} (de época anterior)")
                print(f"NDCG@10 combinado: {combined_ndcg:.4f} (de época anterior)")
