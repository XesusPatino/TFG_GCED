...
class DyadicRegressionDataModule(LightningDataModule):
    ...
    def _check_gender_availability(self):
        ...
        try:
            ...
            try:
                ...
                elif self.dataset_name == "GDSC1":
                    # For GDSC1, check the main CSV file
                    data_file = f"{self.data_dir}/gdsc1_processed.csv"
                    sample_data = pd.read_csv(data_file, nrows=1)
                    return "gender" in sample_data.columns
                else:
                    return False
    ...
    def _load_data_without_gender(self):
        """Load data when no gender information is available (like CTRPV or GDSC1)"""
        ...
        # For GDSC1, we keep only the required columns for recommendation system
        # (but preserve the additional columns for potential future use)
        if self.dataset_name == "GDSC1":
            print(f"   Additional columns available: {[col for col in self.all_data.columns if col not in required_cols]}")
