...
class CollaborativeFilteringModel(LightningModule):
    ...
    def __init__(
        ...
        # Gender-based metrics only if gender data is available
        if self.has_gender:
    ...
    def training_step(self, batch, batch_idx):
        ...
        # Only compute fairness metrics if gender data is available
        if self.has_gender:
    ...
    def validation_step(self, batch, batch_idx, dataloader_idx=None):
        ...
        # Gender-specific metrics only if gender data is available
        if self.has_gender:
        ...
        if self.has_gender:
            ...
            # Log NaN values for gender-specific metrics when no gender data
            self.log("val_rmse_male", torch.tensor(float('nan')), on_step=False, on_epoch=True, prog_bar=True)
            self.log("val_rmse_female", torch.tensor(float('nan')), on_step=False, on_epoch=True, prog_bar=True)
            self.log("val_eo", torch.tensor(float('nan')), on_step=False, on_epoch=True, prog_bar=True)
    ...
    def test_step(self, batch, batch_idx, dataloader_idx=None):
        ...
        # Only log fairness metrics if gender data is available
        if self.has_gender:
            if hasattr(self, 'test_eo'):
            ...
            # Log NaN values for fairness metrics when no gender data
            self.log("test_eo", torch.tensor(float('nan')), on_step=False, on_epoch=True, prog_bar=True)
            self.log("test_eoi", torch.tensor(float('nan')), on_step=False, on_epoch=True, prog_bar=True)
