...
class FairnessMetrics:
    ...
    def compute_all_metrics(self, test_results: Dict, recommendation_results: Optional[Dict] = None):
        ...
        # Add user history-based metrics if available
        if 'user_histories' in test_results:
            metrics.update(self._compute_history_based_fairness(
                test_results['user_ids'],
                test_results['item_ids'],
                test_results['ratings'],
                test_results['predictions'],
                test_results['genders'],
                test_results['user_histories']
            ))
        
        # 2. Recommendation Fairness Metrics (optional)
    ...
    def _compute_history_based_fairness(self, user_ids, item_ids, ratings, predictions, genders, user_histories):
        """
        Compute fairness metrics that consider user rating history
        
        Args:
            user_ids: Tensor of user IDs
            item_ids: Tensor of item IDs
            ratings: Tensor of ground truth ratings
            predictions: Tensor of predicted ratings
            genders: Tensor of user genders
            user_histories: Dict mapping user_id to their rating history DataFrame
        """
        metrics = {}
        ...
        # Calculate errors
        errors = np.abs(ratings_np - preds_np)
        
        # Analyze errors by history size
        history_sizes = {uid: len(hist) for uid, hist in user_histories.items()}
        user_history_sizes = np.array([history_sizes.get(int(uid), 0) for uid in user_ids_np])
        
        # Define history size bins (adjust as needed)
        bins = [0, 10, 25, 50, 100, np.inf]
        bin_labels = ['1-10', '11-25', '26-50', '51-100', '100+']
        
        # Bin users by history size
        user_bins = np.digitize(user_history_sizes, bins)
        
        # Calculate metrics by gender and history size
        print("\nHistory-Based Fairness Analysis:")
        print(f"{'History Size':<16}{'MAE (F)':>10}{'MAE (M)':>10}{'#F':>8}{'#M':>8}{'Gap':>10}")
        
        for i, bin_label in enumerate(bin_labels):
            bin_mask = user_bins == i + 1
            if not bin_mask.any():
                continue
                
            bin_genders = genders_np[bin_mask]
            bin_errors = errors[bin_mask]
            
            mask_f = bin_genders == 0
            mask_m = bin_genders == 1
            
            if mask_f.sum() < MIN_RATINGS_PER_GENDER or mask_m.sum() < MIN_RATINGS_PER_GENDER:
                continue
                
            mae_f = bin_errors[mask_f].mean()
            mae_m = bin_errors[mask_m].mean()
            gap = mae_f - mae_m
            
            metrics[f'history_gap_{bin_label}'] = gap
            print(f"{bin_label:<16}{mae_f:>10.2f}{mae_m:>10.2f}{mask_f.sum():>8}{mask_m.sum():>8}{gap:>10.2f}")
