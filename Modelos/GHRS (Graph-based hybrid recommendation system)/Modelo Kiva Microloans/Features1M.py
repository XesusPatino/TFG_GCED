...
# Lectura del archivo kiva_ml17.csv
df = pd.read_csv(dataPath + 'kiva_ml17.csv')

# Renombrar columnas para mantener consistencia con el código original
df = df.rename(columns={'user_id': 'UID', 'item_id': 'MID', 'rating': 'rate'})
...
alpha_coefs = [0.001]  # Valores más bajos para datasets pequeños
...
for alpha_coef in alpha_coefs:
    ...
    # Asegurarse de que las columnas no numéricas no interfieran con el escalado
    # Antes de guardar X_train, seleccionamos solo las columnas numéricas
    numerical_columns = ['PR', 'CD', 'CC', 'CB', 'LC', 'AND']
    X_train = df_user[['UID'] + numerical_columns].copy()
    X_train.fillna(0, inplace=True)
    X_train.to_pickle(f"data1m/x_train_Kiva_alpha({alpha_coef}).pkl")
