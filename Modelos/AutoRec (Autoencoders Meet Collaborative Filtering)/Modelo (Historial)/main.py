...
result_path = "C:/Users/xpati/Documents/TFG/Pruebas(Metricas)/AutoRec (Autoencoders Meet Collaborative Filtering)/results/Modelo 2"
...
def modified_run():
    ...
    # Si usamos historial, calculamos RMSE con predicciones ajustadas
    if args.use_history and hasattr(model.model, 'predict_for_user'):
        # Calcular RMSE con predicciones ajustadas por historial
        squared_error_sum = 0
        test_count = 0
        
        print("\nCalculando RMSE con predicciones ajustadas por historial...")
        # Mostrar progreso para usuarios con más valoraciones
        progress_step = max(1, num_users // 20)  # Mostrar progreso cada ~5%
        ...
        for user_id in range(num_users):
            ...
            if np.sum(test_mask_R[user_id]) > 0:
                ...
                # Calcular error cuadrático para las valoraciones de prueba
