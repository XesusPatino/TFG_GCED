...
def train(args, data, show_loss, show_topk):
    # Iniciar el tracker de emisiones de CodeCarbon
    tracker = EmissionsTracker(
        project_name="KGNN_LS_Training",
        output_dir="./codecarbon_results",  # Directorio para guardar los informes
        measure_power_secs=10,  # Intervalo de medición del consumo energético (en segundos)
    ...
    try:
        ...
        # Detener el tracker y guardar los resultados
        tracker.stop()

        # Visualizar los resultados de CodeCarbon
        visualize_emissions_results()


def visualize_emissions_results():
    # Cargar el archivo CSV generado por CodeCarbon
    try:
        results = pd.read_csv("./codecarbon_results/emissions.csv")
        print("\n--- Resultados de Emisiones y Consumo Energético ---")
        print(results[["timestamp", "project_name", "duration", "emissions", "energy_consumed"]])
    except FileNotFoundError:
        print("No se encontró el archivo de emisiones. Asegúrate de que CodeCarbon se haya ejecutado correctamente.")
