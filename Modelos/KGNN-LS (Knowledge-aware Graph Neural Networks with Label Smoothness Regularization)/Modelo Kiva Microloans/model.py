...
class KGNN_LS(object):
    ...
    @staticmethod
    def get_initializer():
        return tf.keras.initializers.GlorotNormal()  # Use GlorotNormal as Xavier initializer
    ...
    def _build_model(self, n_user, n_entity, n_relation):
        ...
        # For rating prediction (1-5), use sigmoid to get 0-1 then scale to 1-5
        self.scores_normalized = tf.sigmoid(self.scores) * 4.0 + 1.0
    ...
    def _build_train(self):
        # base loss - use MSE for regression (rating prediction)
        self.base_loss = tf.reduce_mean(tf.square(self.labels - self.scores_normalized))
        ...
        # LS loss - use MSE for label smoothness as well
        predicted_ratings = tf.sigmoid(self.predicted_labels) * 4.0 + 1.0
        self.ls_loss = tf.reduce_mean(tf.square(self.labels - predicted_ratings))
