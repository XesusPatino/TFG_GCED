...
def load_rating(args):
    ...
    # Handle the netflix dataset specifically
    if args.dataset == 'netflix':
        ...
        # If different, we'll need to adjust the column names
        ...
        elif 'userId' in df.columns and 'movieId' in df.columns and 'rating' in df.columns:
            user_col, item_col, rating_col = 'userId', 'movieId', 'rating'
        elif 'UserID' in df.columns and 'MovieID' in df.columns and 'Rating' in df.columns:
            user_col, item_col, rating_col = 'UserID', 'MovieID', 'Rating'
        else:
            # Try to infer from the first 3 columns
            user_col, item_col, rating_col = df.columns[0], df.columns[1], df.columns[2]
            print(f'Using columns: {user_col}, {item_col}, {rating_col}')
        
        # Map user_ids to consecutive indices starting from 0
        ...
        # Apply mappings
        df['user_id_numeric'] = df[user_col].map(user_id_map)
        df['item_id_numeric'] = df[item_col].map(item_id_map)
        
        # Convert to numpy array using the mapped indices
        rating_np = df[['user_id_numeric', 'item_id_numeric', rating_col]].values
        ...
        print(f'Users: {len(unique_users)} (mapped to consecutive indices)')
...
def load_kg(args):
    ...
    # Handle the netflix dataset - create a minimal knowledge graph
    if args.dataset == 'netflix':
        print('Creating minimal knowledge graph for netflix dataset...')
        
        # For netflix, we'll create a simple identity-based knowledge graph
        ...
        if 'item_id' in df.columns:
            ...
            item_col = df.columns[1]  # Assume second column is item_id
