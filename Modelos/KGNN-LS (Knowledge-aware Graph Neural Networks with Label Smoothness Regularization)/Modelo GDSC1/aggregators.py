...
class SumAggregator(Aggregator):
    def __init__(self, batch_size, dim, dropout=0., act=tf.nn.relu, name=None):
        ...
        with tf.compat.v1.variable_scope(self.name):
            # Use tf.keras.initializers.GlorotNormal() instead of tf.contrib.layers.xavier_initializer()
            self.weights = tf.compat.v1.get_variable(
                shape=[self.dim, self.dim], initializer=tf.keras.initializers.GlorotNormal(), name='weights')
    ...
    def _call(self, self_vectors, neighbor_vectors, neighbor_relations, user_embeddings, masks):
        ...
        output = tf.nn.dropout(output, rate=self.dropout)  # rate instead of keep_prob
