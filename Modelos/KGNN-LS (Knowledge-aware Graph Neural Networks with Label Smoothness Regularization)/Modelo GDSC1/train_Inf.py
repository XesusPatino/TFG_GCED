...
def get_interaction_table(train_data, n_entity):
    # Calculate offset to ensure unique keys
    max_item_id = int(train_data[:, 1].max())
    offset = max_item_id + 1
    ...
    if len(np.unique(keys)) != len(keys):
        print(f"Warning: Found duplicate keys in interaction table!")
        # Remove duplicates by averaging ratings for the same user-item pair
        import pandas as pd
        df = pd.DataFrame({'key': keys, 'value': values})
        df = df.groupby('key')['value'].mean().reset_index()
        keys = df['key'].values.astype(np.int64)
        values = df['value'].values.astype(np.float32)
        print(f"Deduplicated: {len(keys)} unique interactions")
    
    # Use the mean rating as default value instead of fixed 3.0
    default_value = float(np.mean(values))
    print(f"Using default value: {default_value:.3f}")
    
    # Use tf.lookup.StaticHashTable
    interaction_table = tf.lookup.StaticHashTable(
        tf.lookup.KeyValueTensorInitializer(keys, values), default_value=default_value)
