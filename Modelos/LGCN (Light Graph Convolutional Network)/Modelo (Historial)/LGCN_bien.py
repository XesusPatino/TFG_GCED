...
def load_ratings_from_dataframe(ratings_df):
    """
    Carga los ratings reales desde un DataFrame.
    ...
    try:
        ...
        print(f"Cargados {len(ratings_dict)} ratings reales desde el DataFrame")
        return ratings_dict
    except Exception as e:
        print(f"Error al cargar ratings desde DataFrame: {e}. No se pueden calcular RMSE/MAE.")
        return {}

# Función OPTIMIZADA para calcular RMSE
def calculate_rmse(model, test_data, n_users, n_items, user_emb, item_emb, sample_size=1000):
    """
    ...
    for (u, item_id), true_rating in sample_ratings:
        ...
        predicted_rating = model.predict_rating(u, item_id, user_emb, item_emb)
...
def calculate_mae(model, test_data, n_users, n_items, user_emb, item_emb, sample_size=1000):
    """
...
class LightGCNModel(tf.keras.Model):
    ...
    def predict_rating(self, user_id, item_id, user_emb=None, item_emb=None):
        # Si no se proveen embeddings, calcularlos.
    ...
    def _predict_from_history(self, user_id, target_item_id, item_embeddings):
        ...
        # Filtrar el item objetivo si está en el historial
        history = history[history['item_id'] != target_item_id]
        if history.empty:
            return 3.0

        # Vectorización
        hist_item_ids = tf.convert_to_tensor(history['item_id'].values, dtype=tf.int32)
        hist_ratings = history['rating'].values
        
        target_tensor = tf.convert_to_tensor([target_item_id], dtype=tf.int32)
        
        try:
            # Obtener embeddings de una vez
            target_emb = tf.nn.embedding_lookup(item_embeddings, target_tensor)
            hist_embs = tf.nn.embedding_lookup(item_embeddings, hist_item_ids)
            
            # Calcular similaridad de coseno de forma vectorizada
            target_emb_norm = tf.nn.l2_normalize(target_emb, axis=1)
            hist_embs_norm = tf.nn.l2_normalize(hist_embs, axis=1)
            
            # (1, emb_dim) @ (emb_dim, n_hist) -> (1, n_hist)
            similarities_tensor = tf.matmul(target_emb_norm, hist_embs_norm, transpose_b=True)
            similarities = tf.maximum(0.0, similarities_tensor[0]).numpy()

        except Exception:
            return 3.0 # Fallback si algún ID no es válido

        if similarities.size == 0:
            return 3.0
        
        sum_sim = np.sum(similarities)
        if sum_sim > 1e-9: # Evitar división por cero
            weights = similarities / sum_sim
            weighted_rating = np.sum(weights * hist_ratings)
            return float(weighted_rating)
        else:
            # Si no hay items similares, devolver la media de ratings del historial o un valor por defecto
            return np.mean(hist_ratings) if hist_ratings.size > 0 else 3.0
...
def extract_user_histories(train_data, ratings_df=None):
    ...
    if ratings_df is not None and not ratings_df.empty:
        # Agrupar por usuario para eficiencia
        grouped = ratings_df.groupby('user_id')
        for user_id, user_ratings in grouped:
            user_histories[user_id] = user_ratings[['item_id', 'rating']]
        
        # Asegurarse de que todos los usuarios del entrenamiento tengan una entrada
        all_train_users = set(range(len(train_data)))
        missing_users = all_train_users - set(user_histories.keys())
        for user_id in missing_users:
            user_histories[user_id] = pd.DataFrame(columns=['item_id', 'rating'])

    else: # Fallback si no hay ratings_df
        for user_id in range(len(train_data)):
            if train_data[user_id]:
                item_ids = list(train_data[user_id])
                ratings = [1.0] * len(item_ids) # Asumir rating 1.0 si no hay datos
...
def train_lightgcn_with_history(model, train_data, val_data, test_data, n_users, n_items, 
    ...
    for epoch in range(epochs):
        ...
        # Calcular embeddings una vez por época para la evaluación
        user_final_emb, item_final_emb, _, _, _ = model((model.user_embedding, model.item_embedding))
        
        epoch_rmse = calculate_rmse(model, val_data, n_users, n_items, user_final_emb, item_final_emb)
        epoch_mae = calculate_mae(model, val_data, n_users, n_items, user_final_emb, item_final_emb)
    ...
    # Calcular embeddings finales para la prueba
    user_final_emb, item_final_emb, _, _, _ = model((model.user_embedding, model.item_embedding))
    final_rmse = calculate_rmse(model, test_data, n_users, n_items, user_final_emb, item_final_emb, sample_size=5000)
    final_mae = calculate_mae(model, test_data, n_users, n_items, user_final_emb, item_final_emb, sample_size=5000)
...
def run_lightgcn_with_history(ratings_path=None, history_weight=0.3):
    ...
    if ratings_path and os.path.exists(ratings_path):
        try:
            ...
            # Poblar el diccionario global desde el DataFrame
            load_ratings_from_dataframe(ratings_df)
