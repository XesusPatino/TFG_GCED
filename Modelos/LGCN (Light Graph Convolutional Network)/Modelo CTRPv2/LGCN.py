...
result_path = "results_lgcn_ctrpv2"
...
def load_ctrpv2_dataset(csv_file, test_size=0.2, val_size=0.1, random_state=42):
    """
    Carga el dataset CTRPv2 desde un archivo CSV y lo divide en train/test/val
    
    Args:
        csv_file: Ruta al archivo ctrpv2_processed.csv
    ...
    print(f"Cargando dataset CTRPv2 desde {csv_file}...")
...
def calculate_rmse(model, test_data, n_users, n_items, sample_size=1000):
    """
    Calcula el RMSE usando los ratings reales de CTRPv2 - OPTIMIZADA.
    """
    global ratings_dict
    ...
    # Normalización global EFICIENTE para el rango de CTRPv2 (~0.1 a 20.0)
    ...
    for i, ((u, item_id), true_rating) in enumerate(valid_pairs):
        ...
        # Mapeo mejorado a escala del dataset CTRPv2 [~0.1, 20.0]
        if max_score > min_score:
            # Mapear a rango aproximado del dataset
            predicted_rating = 0.1 + 19.9 * ((score - min_score) / (max_score - min_score))
        else:
            predicted_rating = 10.0  # valor neutro en la escala CTRPv2
...
def calculate_mae(model, test_data, n_users, n_items, sample_size=1000):
    """
    Calcula el MAE usando los ratings reales de CTRPv2 - OPTIMIZADA.
    """
    global ratings_dict
    ...
    for i, ((u, item_id), true_rating) in enumerate(valid_pairs):
        ...
        if max_score > min_score:
            # Mapear a rango del dataset CTRPv2
            predicted_rating = 0.1 + 19.9 * ((score - min_score) / (max_score - min_score))
        else:
            predicted_rating = 10.0
...
def train_lightgcn_with_metrics(model, train_data, val_data, test_data, n_users, n_items, batch_size=1024, epochs=10, initial_lr=1e-2, k=20):
    ...
    emissions_tracker = EmissionsPerEpochTracker(result_path, "LightGCN_CTRPv2")
    ...
    print("\n" + "="*60)
    print("MÉTRICAS FINALES DEL SISTEMA - CTRPv2")
    print("="*60)
    ...
    plt.title("LightGCN CTRPv2 - Training Loss")
    plt.xlabel("Epoch")
    plt.ylabel("Loss")
    plt.grid(True)
    plt.legend()
    ...
    plt.title("LightGCN CTRPv2 - RMSE")
    plt.xlabel("Epoch")
    plt.ylabel("RMSE")
    plt.grid(True)
    plt.legend()
...
# Función principal MODIFICADA para ejecutar todo con CTRPv2
def run_lightgcn_with_metrics():
    # Ruta del archivo CTRPv2
    csv_file = 'C:/Users/xpati/Documents/TFG/ctrpv2_processed.csv'
    
    # Cargar datos de CTRPv2
    print("Cargando dataset CTRPv2...")
    train_data, test_data, val_data, n_users, n_items, ratings_dict = load_ctrpv2_dataset(
    ...
    # Hiperparámetros ajustados para CTRPv2
    N_LAYERS = 2  
    EMBED_DIM = 128  
    DECAY = 1e-4  
    INITIAL_LR = 1e-3  
    ...
    print("\nStarting LightGCN training with CTRPv2 dataset...")
    ...
    print("\nTraining and evaluation completed for CTRPv2!")
