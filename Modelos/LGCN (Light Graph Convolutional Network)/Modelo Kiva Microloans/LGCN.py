...
def load_kiva_dataset(csv_file, test_size=0.2, val_size=0.1, random_state=42):
    """
    Carga el dataset Kiva desde un archivo CSV y lo divide en train/test/val
    
    Args:
        csv_file: Ruta al archivo kiva_ml17.csv
    ...
    print(f"Cargando dataset Kiva desde {csv_file}...")
    ...
    print(f"Usuarios únicos (prestamistas): {n_users}")
    print(f"Items únicos (proyectos): {n_items}")
...
def show_ratings_statistics():
    ...
    print(f"\nEstadísticas de ratings reales (valoraciones Kiva):")
...
def test_rating_usage():
    ...
    print(f"\nVERIFICACIÓN: Muestra de ratings reales que se usarán (Kiva):")
...
def calculate_rmse(model, test_data, n_users, n_items, sample_size=1000):
    """
    Calcula el RMSE usando los ratings reales de Kiva - OPTIMIZADA.
    """
    global ratings_dict
    ...
    # Normalización global EFICIENTE para el rango de Kiva (3.0 a 5.0)
    ...
    for i, ((u, item_id), true_rating) in enumerate(valid_pairs):
        ...
        # Mapeo mejorado a escala del dataset Kiva [3.0, 5.0]
        if max_score > min_score:
            # Mapear a rango del dataset Kiva
            predicted_rating = 3.0 + 2.0 * ((score - min_score) / (max_score - min_score))
        else:
            predicted_rating = 3.16  # valor promedio en la escala Kiva
...
def calculate_mae(model, test_data, n_users, n_items, sample_size=1000):
    """
    Calcula el MAE usando los ratings reales de Kiva - OPTIMIZADA.
    """
    global ratings_dict
...
def train_lightgcn_with_metrics(model, train_data, val_data, test_data, n_users, n_items, batch_size=1024, epochs=10, initial_lr=1e-2, k=20):
    ...
    emissions_tracker = EmissionsPerEpochTracker(result_path, "LightGCN_Kiva")
    ...
    print("\n" + "="*60)
    print("MÉTRICAS FINALES DEL SISTEMA - KIVA")
    print("="*60)
    ...
    plt.title("LightGCN Kiva - Training Loss")
    plt.xlabel("Epoch")
    plt.ylabel("Loss")
    plt.grid(True)
    plt.legend()
...
# Función principal MODIFICADA para ejecutar todo con Kiva
def run_lightgcn_with_metrics():
    # Ruta del archivo Kiva
    csv_file = 'C:/Users/xpati/Documents/TFG/kiva_ml17.csv'
    
    # Cargar datos de Kiva
    print("Cargando dataset Kiva...")
    train_data, test_data, val_data, n_users, n_items, ratings_dict = load_kiva_dataset(
    ...
    print(f"Number of Users (prestamistas): {n_users}")
    print(f"Number of Items (proyectos): {n_items}")
    ...
    # Hiperparámetros ajustados para Kiva
    N_LAYERS = 2  
    EMBED_DIM = 64  
    DECAY = 1e-4  
    INITIAL_LR = 1e-3  
    ...
    print("\nStarting LightGCN training with Kiva dataset...")
    ...
    print("\nTraining and evaluation completed for Kiva!")
