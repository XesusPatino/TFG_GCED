...
result_path = "results_lgcn_dots2023"
...
def load_dots2023_dataset(csv_file, test_size=0.2, val_size=0.1, random_state=42):
    """
    Carga el dataset DOTS 2023 desde un archivo CSV y lo divide en train/test/val
    
    Args:
        csv_file: Ruta al archivo dot_2023_processed.csv
    ...
    print(f"Cargando dataset DOTS 2023 desde {csv_file}...")
    ...
    print(f"Usuarios únicos (países): {n_users}")
    print(f"Items únicos (indicadores): {n_items}")
    print(f"Rating promedio (valor del indicador): {df['rating'].mean():.3f}")
    print(f"Rating mínimo (valor del indicador): {df['rating'].min():.3f}")
    print(f"Rating máximo (valor del indicador): {df['rating'].max():.3f}")
...
def show_ratings_statistics():
    ...
    print(f"\nEstadísticas de ratings reales (valores DOTS 2023):")
...
def test_rating_usage():
    ...
    print(f"\nVERIFICACIÓN: Muestra de ratings reales que se usarán (DOTS 2023):")
    for (u, item_id), rating in sample_ratings:
        print(f"  Usuario {u} (País), Item {item_id} (Indicador) → Rating real: {rating:.3f}")
...
def calculate_rmse(model, test_data, n_users, n_items, sample_size=1000):
    """
    Calcula el RMSE usando los ratings reales de DOTS 2023 - OPTIMIZADA.
    """
    global ratings_dict
    ...
    # Tomar muestra más grande para estabilidad (ajustada para dataset pequeño)
    ...
    # Normalización global EFICIENTE para el rango de DOTS 2023 (0.0 a 12.08)
    ...
    for i, ((u, item_id), true_rating) in enumerate(valid_pairs):
        ...
        # Mapeo mejorado a escala del dataset DOTS 2023 [0.0, 12.08]
        if max_score > min_score:
            # Mapear a rango del dataset DOTS 2023
            predicted_rating = 0.0 + 12.08 * ((score - min_score) / (max_score - min_score))
        else:
            predicted_rating = 6.78  # valor promedio en la escala DOTS 2023
...
def calculate_mae(model, test_data, n_users, n_items, sample_size=1000):
    """
    Calcula el MAE usando los ratings reales de DOTS 2023 - OPTIMIZADA.
    """
    global ratings_dict
...
def train_lightgcn_with_metrics(model, train_data, val_data, test_data, n_users, n_items, batch_size=1024, epochs=10, initial_lr=1e-2, k=20):
    ...
    emissions_tracker = EmissionsPerEpochTracker(result_path, "LightGCN_DOTS2023")
    ...
    print("\n" + "="*60)
    print("MÉTRICAS FINALES DEL SISTEMA - DOTS 2023")
    print("="*60)
    ...
    plt.title("LightGCN DOTS 2023 - Training Loss")
    plt.xlabel("Epoch")
    plt.ylabel("Loss")
    plt.grid(True)
    plt.legend()
    ...
    plt.title("LightGCN DOTS 2023 - RMSE")
    plt.xlabel("Epoch")
    plt.ylabel("RMSE")
    plt.grid(True)
    plt.legend()
    ...
    plt.title("LightGCN DOTS 2023 - MAE")
    plt.xlabel("Epoch")
    plt.ylabel("MAE")
    plt.grid(True)
    plt.legend()
...
# Función principal MODIFICADA para ejecutar todo con DOTS 2023
def run_lightgcn_with_metrics():
    # Ruta del archivo DOTS 2023
    csv_file = 'C:/Users/xpati/Documents/TFG/dot_2023_processed.csv'
    
    # Cargar datos de DOTS 2023
    print("Cargando dataset DOTS 2023...")
    train_data, test_data, val_data, n_users, n_items, ratings_dict = load_dots2023_dataset(
    ...
    print(f"Number of Users (países): {n_users}")
    print(f"Number of Items (indicadores): {n_items}")
    ...
    # Hiperparámetros ajustados para DOTS 2023 (dataset pequeño)
    N_LAYERS = 2  
    EMBED_DIM = 64  
    DECAY = 1e-4  
    INITIAL_LR = 1e-3  
    EPOCHS = 50 # Menos épocas para dataset pequeño
    BATCH_SIZE = 256  # Batch size más pequeño
    ...
    print("\nStarting LightGCN training with DOTS 2023 dataset...")
    ...
    print("\nTraining and evaluation completed for DOTS 2023!")
